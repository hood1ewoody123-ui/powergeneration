...
import { CampBlock, campFormButtonClassName, CAMP_MIN } from '@/lib/camp-minimal'
...
const toneStyles: Record<
  PricingTicketTone,
  { headline: string; meta: string; price: string; body: string }
> = {
  green: {
    headline: CAMP_MIN.monoBright,
    meta: CAMP_MIN.mono,
    price: 'font-display text-[clamp(1.5rem,3vw,2.5rem)] font-bold leading-none tracking-tight text-green',
    body: CAMP_MIN.body,
  },
  red: {
    headline:
...
export function PricingTickets() {
  return (
    ...
    >
      <ul className="mt-14 flex flex-col gap-12 md:gap-16">
          ...
          return (
            <li
              ...
              className={cn(
                'flex flex-col gap-4',
                ticket.description
                  ? 'sm:flex-row sm:items-start sm:justify-between sm:gap-12'
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                {ticket.description ? (
                  <p className={cn(tone.body, 'mt-6')}>{ticket.description}</p>
                ) : null}
              </div>
      ...
      <Link
        href="#register"
        className={cn(campFormButtonClassName(), 'mt-10')}
      >
        Занять место
      </Link>
    </CampBlock>
