...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const webhookUrl =
      process.env.BATTLE_REGISTER_WEBHOOK_URL ??
    ...
    if (webhookUrl) {
      ...
      if (!webhookRes.ok) {
        console.error('[battle-register] webhook failed', webhookRes.status)
        return NextResponse.json(
      ...
      console.info('[battle-register] new application', data)
    }
